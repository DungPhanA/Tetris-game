...
class Block(pg.sprite.Sprite):
    ...
    def sfx_end_time(self):
        if self.tetromino.tetris.app.anim_trigger:
            self.cycel_counter += 1
            if self.cycel_counter > self.sfx_cycles:
                self.cycel_counter = 0
                return True
...
class Tetromino:
 ...
 #khi xoay tạo block mới và lưu lại       
        if not self.is_collide(new_block_positions):
            # Nếu không có va chạm, cập nhật vị trí mới của các khối
